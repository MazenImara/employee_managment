...
class Database{
    public:
        ...
        list<Task> selectTasksByEmployeeId(string id){
            list<Task> tasks;
            string query ="SELECT  task.project_id ,task.id,task.title,task.status,task.startTemp,task.endTemp,task.timeSpend,task.employee_id from  task where task.employee_id="+id;
            const char* q = query.c_str();
            qstate = mysql_query(conn,q);
            if(!qstate)
            {
